...
let BlueGalaxy = {
    ...
    CalcBody: () => {
        ...
        for (let i = 0; i < CityMap.length; i++) {
            ...
            if (CityEntity.state.production) {
                ...
                CityEntity.state.production.forEach(product => {
                    ...
                    /* preserve start */
                    if (product.resources?.premium)
                        Diamonds += product.resources.premium
                    else if (product.type == "genericReward" && product.resources?.subType == "premium")
                        Diamonds += product.resources.amount;
                    /* preserve end */
                ...
                if (GoodsSum > 0 || FP > 0 || Diamonds > 0 || FragmentAmount > 0 || OlderGoodsSum > 0) {    /* preserve */
        ...
        if (BlueGalaxy.DoubleCollections > 0)
            Buildings = Buildings.filter(obj => ((obj['FP'] > 0 || obj['Diamonds'] > 0 || obj['Goods'] > 0 || obj['GuildGoods']>0 || obj['FragmentAmount']>0) && obj['In'] < 23.5 * 3600)); // Hide everything above 23h
        ...
        Buildings = Buildings.sort(function (a, b) {
            if (BlueGalaxy.sort.col) {
                ...
                return (b['Diamonds'] - a['Diamonds']) + (b['FP'] - a['FP']) + BlueGalaxy.GoodsValue * (b['Goods'] - a['Goods'] + (showBGFragments ? (b['FragmentAmount'] - a['FragmentAmount'])*3 : 0));
        ...
        table.push('<thead>' +
            ...
            '<th class="is-number icon diamonds ' + (BlueGalaxy.sort.col=="Diamonds" ? BlueGalaxy.sort.order : "") + '" title="' + i18n('Boxes.BlueGalaxy.Diamonds') + '" data-type="bg-group" data-colname="Diamonds"><span></span></th>' +   /* preserve */
        ...
        for (let i = 0; i < 50 && i < Buildings.length; i++) { // limits the list to max 50 items
            ...
            table.push('<td class="text-center" data-number="'+Buildings[i].Diamonds+'">' + HTML.Format(Buildings[i]['Diamonds']) + '</td>');  /* preserve */
